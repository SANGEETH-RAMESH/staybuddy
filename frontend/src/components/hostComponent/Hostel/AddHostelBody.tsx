...
const HostelForm = () => {
  ...
  const handleSubmit = async () => {
    try {
      ...
      if (formData.facilities) {
        const selectedFacilities = Object.keys(formData.facilities).reduce(
          (acc, key) => {
            if (formData.facilities[key as keyof typeof formData.facilities]) {
              acc[key] = true;
            }
            return acc;
          },
  ...
  return (
    <div className="w-full p-6 bg-white rounded-lg shadow-md">
      <div className="mb-6">
        <h2 className="text-2xl font-bold text-gray-800 mb-2">Add New Hostel</h2>
        <p className="text-gray-600">Fill in the details to list your hostel property</p>
      </div>
