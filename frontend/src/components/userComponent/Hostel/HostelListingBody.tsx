  ...
  const fetchHostels = async (page: number = 1, limit: number = ITEMS_PER_PAGE,
    ...
    try {
      ...
      const formattedHostels = hostelData.map((item: Hostel) => {
        ...
        let facilitiesObj: Facilities = { wifi: false, food: false, laundry: false };

        if (item.facilities && typeof item.facilities === 'object' && !Array.isArray(item.facilities)) {
          facilitiesObj = {
            wifi: !!(item.facilities as Facilities).wifi,
            food: !!(item.facilities as Facilities).food,
            laundry: !!(item.facilities as Facilities).laundry,
          };
        }
        ...
        <div className="flex flex-col lg:flex-row gap-4 lg:gap-6">
          ...
          <div className="flex-1 order-1 lg:order-2">
            ...
            <>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4 sm:gap-6 min-h-[300px]">
                {loading && !isSearching ? (
