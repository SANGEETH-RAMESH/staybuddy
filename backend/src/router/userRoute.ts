...
const userController = new UserController(userService,userService,userService);


user_route.post('/auth/signup',validate(signupValidation) ,userController.userSignUp.bind(userController))
user_route.post('/auth/verify-otp',validate(otpValidation) ,userController.verifyOtp.bind(userController))
user_route.post('/auth/login',validate(signInValidation), userController.verifyLogin.bind(userController))
user_route.post('/auth/resend-otp', userController.resendOtp.bind(userController))
user_route.post('/auth/forgot-password',validate(forgotPasswordValidation), userController.forgotPassword.bind(userController))
user_route.post('/auth/verify-forgot-otp', userController.verifyForgotPasswordOtp.bind(userController))
user_route.post('/auth/reset-password',validate(resetPasswordValidation) ,userController.resetPassword.bind(userController))
...
// user_route.patch('/profile', validate(profileUpdateValidation),userAuthMiddleware, userController.editUserDetail.bind(userController));
